...
class FinetuneDataset(Dataset):
    def __init__(self, config_path, transform, max_words=30, image_words=257, tokenizer_path=None,args=None):
        self.args = args
        ...
        if not self.args.teacher_forcing:
    ...
    def __getitem__(self, index):
        ...
        if self.args.teacher_forcing:
            ...
            input2_mask_list = []
            image = None
            for time_ind in range(self.args.max_action_len):

                if time_ind < len(list(data_item['data'].keys())):
                    data = data_item['data'][str(time_ind)]
                    answer = data['output']

                    input1_text = data['instruction']
                    input1 = torch.tensor(self.tokenizer.encode(input1_text, bos=True, eos=False), dtype=torch.int64)
                    input1_length = len(input1)

                    while input1_length >= self.max_words:
                        input1_text = input1_text[:int(2*len(input1_text)/3)]
                        input1 = torch.tensor(self.tokenizer.encode(input1_text, bos=True, eos=False), dtype=torch.int64)
                        input1_length = len(input1)

                    input2 = input1_text + answer
                    input2_ori = torch.tensor(self.tokenizer.encode(input2, bos=True, eos=True), dtype=torch.int64)
                    ...
                    input2_mask_list.append(input2_mask)

                else:
                    data = data_item['data'][str(len(list(data_item['data'].keys()))-1)]
                    answer = data['output']
                    input1_text = data['instruction']

                    input1 = torch.tensor(self.tokenizer.encode(input1_text, bos=True, eos=False),
                                          dtype=torch.int64)
                    input1_length = len(input1)
                    while input1_length >= self.max_words:
                        input1_text = input1_text[:int(2 * len(input1_text) / 3)]
            ...
            input2_list = torch.stack(input2_list)
            labels_list = torch.stack(labels_list)
            input2_mask_list = torch.stack(input2_mask_list)

            if image is None:
                return input2_list, labels_list, input2_mask_list
            else:
                return input2_list, labels_list, input2_mask_list, image
        else:
