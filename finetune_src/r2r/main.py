...
from pretrain_src.model.beit.modeling_discrete_vae import Dalle_VAE


from utils.distributed import init_distributed, is_default_gpu, is_dist_avail_and_initialized
...
def build_dataset(args, rank=0, is_test=False):
    ...
    tok = None
    feat_db_train = ImageFeaturesDB(args.img_ft_file, args.image_feat_size)
    ...
    if args.aug is not None:
        aug_instr_data = construct_instrs(
            args.anno_dir, args.dataset, [args.aug], tokenizer=tok, max_instr_len=args.max_instr_len,args=args
        )
        aug_env = dataset_class(
            feat_db_train, aug_instr_data, args.connectivity_dir, batch_size=args.batch_size,
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
            sel_data_idxs=None, name='aug', args=args
        )
    else:
        aug_env = None
    ...
    if args.llm_predict:
        val_env_names = ['val_unseen_subset', 'val_unseen','val_seen','val_train_seen']
    ...
    for env_name, env in val_envs.items():
        ...
        if default_gpu:
            if 'test' not in env_name:
                if args.step_eval:
                    score_summary, _, step_sr, step_spl = env.eval_metrics(preds, step_eval=True)
                    print(step_sr, step_spl)
                else:
                    score_summary, _ = env.eval_metrics(preds)
