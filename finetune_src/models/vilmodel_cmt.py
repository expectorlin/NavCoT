...
class ImageEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.visual_encoder = VisualEncoder(config.image_feat_size)

    def forward(self, img_feat, ang_feat, type_embeddings, nav_types=None):
        img_feat = self.visual_encoder(img_feat)
...
class HistoryEmbeddings(nn.Module):
    ...
    def forward(self, img_feats, ang_feats, pos_ids, 
        ...
        img_feats = self.visual_encoder(img_feats)
        ...
        if self.pano_encoder is not None:
            pano_img_feats = self.visual_encoder(pano_img_feats)
...
class NavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if config.use_ig:
            if config.weighted_token:
                self.mapwig_head = WeightedActionPredictionWithImageGeneration(config.hidden_size, config.ig_head, 14*14)
            else:
                self.apwig_head = ActionPredictionWithImageGeneration(config.hidden_size, config.ig_head)
    ...
    def forward(self, mode, txt_ids=None, txt_embeds=None, txt_masks=None,
                ...
                ob_masks=None, position_id=None):
        ...
        # cross-modal encoding per step
            ...
            if self.config.use_ig:
                if self.config.weighted_token:
                    ig_probs = self.mapwig_head(txt_embeds[:, 0], position_id)
                else:
                    ig_probs = self.apwig_head(txt_embeds[:, :1])
                return act_logits, txt_embeds, hist_embeds, ob_embeds, ig_probs
            else:
